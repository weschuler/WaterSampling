...
#-------- Check using longitude, latitude, altitude using great circle distance.
#        dxy, dz = self.distance_to_wp(lat,lon,alt)
...
# ------------ Uncomment this section while flying outdoors
...
# ------------ Uncomment this section while flying in mocap
        target = np.array(([x], [y], [z]))
        current = np.array(([self.local_position.pose.position.x],
                        [self.local_position.pose.position.y],\
                        [self.local_position.pose.position.z]))

        dxy = np.linalg.norm(target[0:2] - current[0:2])
#-----------------------------------------------------------------------------        
#        if dxy > self.radius:
...
# When using the F550 inside MoCap
                if self.sampling_flag_data == 0:
                    self.reached = False
